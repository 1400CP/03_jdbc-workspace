...
public class ProductController {
	...
	public void insertStatus(String productId, String amount, String status) {
		ProductIO pd = new ProductIO();
		pd.setProductId(productId);
		pd.setAmount(Integer.parseInt(amount));
		pd.setStatus(status);
		
		int result = new ProductService().insertStatus(pd);
	...
	//case6_5 상품 출고 => insert
	public void outputStatus(String productId, String amount, String status) {
		int result = new ProductService().outputStatus(productId, amount, status);
		
		if(result > 0) {
			new ProductMenu().displaySuccess("상품 출고!");
		}else {
			new ProductMenu().displayFail("실패. 재시도.");
		}
